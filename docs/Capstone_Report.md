...
![Figure 4.2](../outputs/forecast_scoreboard_exante.png)

*Figure 4.2. The demand-model scoreboard on the single 2019–2023 hold-out. Bars show the ex-ante error each model incurs when it must forecast its own drivers and propagate the demand lag recursively; hollow markers show the conditional backcast on observed drivers, kept only as the optimistic reference. Single-country models cluster near 9–10% with negative hold-out R² against the deliberate post-2018 structural break; the pooled Central-Asia models lead at about 6% and turn R² positive. Source: forecast_scoreboard_advanced.csv (project Notebook 07).*

**Table 4.1. Demand models on the 2019–2023 hold-out (single test window).** Ex-ante is the headline; the conditional backcast is reported for reference only. Model skill is scored by mean absolute percentage error (MAPE), read against conventional forecast-accuracy bands (Lewis, 1982).
...
Figure 4.3 makes that negative R² legible rather than abstract. It plots actual 2019–2023 demand against the ex-ante hold-out predictions of the four single-country driver models, each forecasting its own drivers and propagating the demand lag recursively. The actual series steps up sharply — the post-2018 surge compounded by the 2021 rebound — while every model, trained only on the smooth 1990–2018 record, tracks a far gentler path and under-shoots the 2023 actual by roughly **9 TWh, about 12%**. The shortfall is not scatter around the actual but a one-sided gap below it, in the same direction for every model: that systematic under-prediction of a regime the training window never contained is exactly what a negative hold-out R² against a structural break looks like.

![Figure 4.3](../data/processed/forecast_holdout_actuals.png)

*Figure 4.3. Ex-ante hold-out — actual electricity demand versus the four single-country models' 2019–2023 predictions. Each model forecasts its own drivers and propagates the demand lag recursively, so none is handed observed hold-out inputs; the shaded band spans the four models. All sit well below the post-2021 actual, under-shooting by ~9 TWh (~12%) at 2023 — the visible source of the negative hold-out R² in Table 4.1. Source: forecast_holdout_predictions.csv (reproduces the project Notebook 07 ex-ante machinery).*
...
The exploratory work gave no reason to expect a tight forecast — thirty observations, a structural break in 2017–18, and drivers that must themselves be projected all argue for a wide band — and the fan is correspondingly wide. At 2030 the model's 90% predictive band runs from **76.7 to 95.3 TWh** around the 86.0 central value (roughly ±11%); extended to 2040 the published fan reaches **[110, 137] TWh at 80% and [103, 145] TWh at 95%** confidence [FN: From the project's GitHub repository: forecast_demand.csv (Notebook 07 §8).]. 
...
The first signal must be stated carefully, because its construction invites an overclaim the report declines to make. The deficit analysis compares required generation (demand grossed up for losses) against planned supply — but planned supply is *set equal* to the central required generation, so at the central demand path the system balances by construction and the point deficit is zero in every year [FN: From the project's GitHub repository: investment-signal model, investment_signal_deficit.csv (Notebook 08).]. There is therefore no forecast "supply gap" along the central path, and the report does not claim one. The genuine signal is the **reserve headroom the demand uncertainty implies**: at the upper-80% demand band, required generation exceeds the central plan by about **6.5 TWh in 2024, widening to 14.8 TWh by 2040** [FN: From the project's GitHub repository: investment-signal model, investment_signal_deficit.csv (Notebook 08).]. That is the capacity margin the build-out must carry to stay adequate against a plausibly higher load — a reserve-planning quantity, identical across the three scenarios because it is driven by the shared demand fan rather than by build-out ambition. The year-by-year "deficit/stress" flags in the underlying table alternate on rounding noise around a zero point estimate and should not be read as a calendar of shortfalls; the substantive number is the widening reserve requirement.

## 6.3 Signal 2 — Gas-import exposure

The second signal makes Chapter 2's gas-dependence concrete and forward-looking. Gas burned for power generation stands at about **17 bcm in 2024**, and its 2040 value is a direct function of build-out: it **rises to 24.3 bcm under BAU**, falls to about **8.9 bcm under Government** and stays roughly flat thereafter, and falls further to about **4.5 bcm under Accelerated**, declining steadily through 2040 (gas is assumed to supply 88% of thermal output) [FN: From the project's GitHub repository: investment-signal model, investment_signal_gas.csv (Notebook 08).]. The exposure this measures is both fiscal and strategic: Uzbekistan is a net gas exporter that turns net importer in winter (§2.6), so every bcm burned for domestic power is a bcm not exported and, at the winter margin, a bcm imported. The BAU path therefore does not merely emit more carbon; it deepens the country's exposure to gas-price and gas-supply shocks at exactly the season the grid is tightest, while the Accelerated path cuts gas-for-power by roughly three-quarters and insulates the system. Renewable build-out reads, in this signal, as energy-security policy as much as climate policy.

That power-sector draw lands on a national gas balance that is itself tightening, and the wider picture sharpens the signal (Figure 6.2). On the StatSUZ energy-balance basis, gas production has slid from comfortably above domestic demand — a self-sufficiency ratio of about **125% in 2010** — to roughly **88% in 2024**, with output dropping below domestic consumption for the first time around **2023**. The country that built its power system on cheap surplus gas now sits, on its own production trend, below self-sufficiency. This is the macro fact beneath the power-sector exposure above: once the overall balance crosses under 100%, every incremental bcm the BAU grid burns for electricity is drawn from a pool that no longer covers domestic needs, so the fiscal cost and the winter-supply risk both bind harder than the power-sector figures alone suggest. Renewable build-out, read against this trend, is the most direct lever the power sector has on national gas security.

![Figure 6.2](../data/processed/gas_production_consumption.png)

*Figure 6.2. National gas production versus domestic demand (left, PJ) and the self-sufficiency ratio (right, production ÷ demand), on the StatSUZ energy-balance basis, 2010–2024. Self-sufficiency falls from ~125% in 2010 to ~88% in 2024, crossing below 100% around 2023 — the onset of structural import exposure at the margin. Source: master_dataset.csv, StatSUZ energy balances (ktoe→TJ→PJ) (StatSUZ, 2024).*

## 6.4 Signal 3 — The persistence of grid losses

The third signal is the loss rate of §6.1 read as a *trend* rather than a level: losses have not merely been high, they have been stubbornly stable near 16% for two decades, with no downward trajectory in the confirmed record (Figure 6.1) [FN: From the project's GitHub repository: investment-signal model, investment_signal_td.csv (Notebook 08).]. Persistence is itself the signal. A loss rate that drifted down would suggest the problem is being solved by incremental maintenance; a loss rate pinned at 16% across twenty years of rising demand indicates chronic underinvestment in transmission and distribution that ordinary operations will not fix. This is the demand-side complement to the supply-side opportunity: the same network that loses a sixth of its throughput is the network that must absorb a west-to-east renewable build and a five-fold swing in possible 2040 carbon intensity. The persistence finding is what turns grid modernization from a maintenance line item into a structural advisory engagement.

## 6.5 Capital envelopes, mapped to the advisory categories

The signals above imply capital, and the model sizes it — with a caveat that must travel with every figure. The capex numbers are **undiscounted, constant-cost upper-bound envelopes**: unit costs multiplied by capacity added, with no discounting, no learning-curve cost decline, and no financing structure. They bound the capital mobilised; they are not net-present-value estimates, and they should be read as the high end of a range [FN: From the project's GitHub repository: investment-signal model, investment_signals.csv (Notebook 08).]. On that basis the total 2024–2040 build-out envelope is about **$27.3 bn under BAU, $56.9 bn under Government, and $79.3 bn under Accelerated**, with wind and solar the largest line items in every scenario (Government: wind $16.0 bn, solar $13.1 bn, thermal $12.1 bn, hydro $8.4 bn, transmission $6.2 bn, storage $1.1 bn) [FN: From the project's GitHub repository: investment-signal model, investment_signals.csv (Notebook 08).] (Figure 6.3).

![Figure 6.3](../data/processed/forecast_investment.png)

*Figure 6.3. Capital envelope by technology and scenario, 2024–2040 (undiscounted, constant-cost upper bounds). Total build-out runs ~$27.3 bn (BAU), ~$56.9 bn (Government), ~$79.3 bn (Accelerated); wind and solar dominate. Source: investment_signals.csv (project Notebook 08).*

Mapped to the five categories through which an engineering-advisory practice would engage, the Government-path programme falls out as follows (project Notebook 08 §15):
...
- Asian Development Bank. (n.d.). *Central Asia Power System: Operational history and thermal decommissioning* [locator to confirm].
- Box, G. E. P., Jenkins, G. M., Reinsel, G. C., & Ljung, G. M. (2015). *Time series analysis: Forecasting and control* (5th ed.). Wiley.
- Central Asia Regional Economic Cooperation Program (CAREC). (2024). *Oblast wind and solar resource assessment* [locator to confirm].
- DNV. (2025). *Energy Transition Outlook 2025: A global and regional forecast to 2060*. DNV AS. https://www.dnv.com/eto
- Enerdata. (n.d.). *Uzbekistan country energy profile* [locator to confirm].
- Eurasian Development Bank. (2026). *Central Asia energy outlook 2026* [locator to confirm].
- Eurasian Research Institute. (n.d.). *Uzbekistan power-sector commentary* [locator to confirm].
- European Bank for Reconstruction and Development. (n.d.). *Uzbekistan thermal-generation project documents* [locator to confirm].
- Global Energy Monitor. (n.d.). *Global Integrated Power Tracker: Uzbekistan* [locator to confirm].
- Global Solar Atlas. (n.d.). *Solar resource data: Uzbekistan* [Data set; locator to confirm].
- Government of Uzbekistan. (2018). *Presidential resolution PP-3981 on measures for the further development of renewable energy* [locator to confirm].
- Government of Uzbekistan. (2019). *Presidential resolution PP-4249 on the restructuring of JSC Uzbekenergo* [locator to confirm].
- Government of Uzbekistan. (2024). *Amendment to the Law on the Use of Renewable Energy Sources* [locator to confirm].
...
- International Energy Agency. (2024a). *Central Asia energy outlook* [locator to confirm]. IEA.
- International Energy Agency. (2024b). *Greenhouse gas emissions from energy* [Data set; locator to confirm]. IEA.
- International Energy Agency. (2024c). *Solar PV and renewable energy deployment* [locator to confirm]. IEA.
- International Energy Agency. (2024d). *Uzbekistan* [Energy profile]. IEA. https://www.iea.org/countries/uzbekistan
- International Energy Agency. (2024e). *World energy balances* [Data set; locator to confirm]. IEA.
...
- Jamestown Foundation. (n.d.). *Central Asia energy and security analysis* [locator to confirm].
- Lewis, C. D. (1982). *Industrial and business forecasting methods*. Butterworth Scientific.
- Lundberg, S. M., & Lee, S.-I. (2017). A unified approach to interpreting model predictions. In *Advances in Neural Information Processing Systems 30* (pp. 4765–4774).
- Minnebo. (n.d.). *Uzbekistan: Country energy report*. Joint Research Centre, European Commission [locator to confirm].
- National Statistics Committee of the Republic of Uzbekistan (StatSUZ). (2024). *National electricity and fuel–energy balance* [Data set; locator to confirm].
- Open-Meteo. (n.d.). *Historical weather API (reanalysis data)* [Data set; locator to confirm].
- Our World in Data. (n.d.). *Energy* [Data set; locator to confirm].
- Power Technology. (n.d.). *Uzbekistan power-plant profiles* [locator to confirm].
- Rosatom. (2025). *Jizzakh nuclear power plant: Project record* [locator to confirm].
- tep.uz. (n.d.). *Thermal power station profiles* [locator to confirm].
- Times of Central Asia. (n.d.). *Winter electricity imports and capacity-utilisation reporting* [locator to confirm].
...
- World Bank. (2025). *Electricity Distribution Improvement Project (P504630)* [locator to confirm].
- World Nuclear News. (n.d.). *Uzbekistan: Nuclear power project updates* [locator to confirm].
...
*The deployed Bayesian-ridge model is estimated in standardised levels with shrinkage and reports a predictive standard deviation rather than per-coefficient significance; its weights are likewise non-interpretable.*
...
## Appendix G — Supplementary energy-system analysis
...
**Economy-wide carbon and the gas signature.** The power sector's gas dependence is a microcosm of the whole economy's. Natural gas accounts for about **73% of Uzbekistan's economy-wide combustion CO₂** in the most recent year, with oil and a thin coal slice making up the remainder, so the decarbonisation lever the report identifies for electricity — displacing gas with built renewables — is also the dominant lever for the national emissions total (Figure G.1). The per-capita series adds a second, more encouraging fact: economy-wide CO₂ per person has fallen from about **5.8 tCO₂ in 1990 to 3.4 tCO₂** in the latest year, a decline driven by the post-Soviet restructuring and the energy-intensity improvement documented in G.4 rather than by any fuel switch. The level remains gas-bound; the trend is downward.

![Figure G.1](../data/processed/appendixG_co2_economy.png)

*Figure G.1. Economy-wide CO₂ from fuel combustion by source (left, MtCO₂) and per capita (right, tCO₂), 1990–2023. Natural gas drives ~73% of combustion CO₂; per-capita emissions fall from ~5.8 to ~3.4 tCO₂. Source: IEA Greenhouse Gas Emissions from Energy and IEA Energy Transitions Indicators (IEA, 2024b).*

**Liquids: a shrinking, condensate-tilting pool.** Uzbekistan's liquid-fuels base has contracted sharply over the forecast-relevant period. Crude-oil output has fallen about **65% since 2010**, and gas condensate is now the larger component of a shrinking total liquids pool — a structural shift that leaves the refined slate (diesel, gasoline, fuel oil) dependent on a narrowing domestic feedstock (Figure G.2). This sits alongside the gas-balance signal of §6.3: the same maturing upstream that pushes gas self-sufficiency below 100% also thins the liquids supply, so the country's hydrocarbon base is narrowing on two fronts at once, reinforcing the strategic case for electrification and renewables.

![Figure G.2](../data/processed/appendixG_liquids.png)

*Figure G.2. Liquids production 2010–2024 (left, thousand tonnes: crude oil, gas condensate, and their total) and the 2024 refined slate (right, kt: diesel, gasoline, fuel oil). Crude output falls ~65% since 2010 as condensate becomes the larger share of a shrinking pool. Source: StatSUZ national production volumes (StatSUZ, 2024).*

**Primary energy supply: four-fifths gas.** At the level of total primary energy supply, Uzbekistan is among the most gas-concentrated systems in the world: natural gas supplies roughly **79% of primary energy**, with oil and coal making up most of the rest and non-hydro renewables still a sliver even at the end of the record (Figure G.3). This is the structural fact behind every decarbonisation lever in the report — a system this gas-bound cannot decarbonise by re-dispatch, only by building alternative supply, which is precisely why the transition reads throughout as a construction story rather than an operational one.

![Figure G.3](../data/processed/appendixG_primary_energy.png)

*Figure G.3. Total primary energy supply by source, 1990–2023 (PJ): natural gas, oil, coal, hydro, biofuels & waste, and solar/wind. Natural gas is ~79% of primary supply throughout; solar and wind remain a sliver even at the end of the record. Source: IEA World Energy Balances (IEA, 2024e).*

![Figure G.4](../data/processed/appendixG_energy_intensity.png)

*Figure G.4. Energy intensity of GDP (left, MJ per 2015 USD) and primary energy per capita (right, GJ per capita), 1990–2023. Intensity falls ~75% and per-capita supply ~40% as the post-Soviet economy de-intensifies. The intensity level is large because it is measured against market-exchange-rate USD GDP, as reported by the IEA; the decline is the salient feature. Source: IEA World Energy Balances (IEA, 2024e).*

**Energy intensity: a steep, sustained de-intensification.** The one unambiguously favourable long-run trend is efficiency. Energy intensity of GDP has fallen about **75% since 1990**, and primary energy per capita is down about **40%** over the same window (Figure G.4) — the economy now extracts far more output from each unit of energy than the late-Soviet system did. This de-intensification is what allows the per-capita emissions decline of G.1 to coexist with sustained economic growth, and it is the macro backdrop against which the report's ~9% forward grid-loss target (§6.1) is plausible: the system has demonstrated, at the economy-wide level, that large efficiency gains are achievable, even as the grid-loss component of that efficiency story remains stubbornly unrealised.

**The four exhibits, read as one.** Taken together, the four panels describe a single energy system caught between a heavy structural inheritance and a genuine, decades-long efficiency gain, and they explain why the report treats Uzbekistan's transition as a construction problem rather than a dispatch problem. The inheritance is gas: roughly four-fifths of primary energy (G.3) and about three-quarters of economy-wide combustion CO₂ (G.1) trace to a single fuel, a concentration no amount of re-dispatch can unwind, because no idle low-carbon capacity waits behind the gas fleet. The upstream that supplies that gas is itself maturing — crude output down about **65% since 2010**, condensate now the larger share of a shrinking liquids pool (G.2) — so the hydrocarbon base is narrowing on two fronts at once even as electricity demand is set to rise. Against that backdrop the efficiency record (G.4) is the one unambiguously favourable trend, and it carries a specific analytical weight: an economy that has already taken roughly three-quarters out of its energy intensity since 1990 has demonstrated, at the macro scale, that the order of efficiency gain the report's grid-loss target assumes is achievable in practice, not merely in principle. The unrealised piece is narrower — the network-loss component specifically, a well-bounded target inside a system that has already proven it can de-intensify. That is the precise sense in which this appendix reinforces the main argument: the demand the model projects, the supply gap the scenarios open, and the loss reduction the investment signals price are all expressions of one structural fact — a gas-bound but intensity-improving system whose decarbonisation must be built rather than dispatched, and whose most credible near-term efficiency win is the grid itself.
